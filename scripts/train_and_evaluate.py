...
Produces:
    ...
    training_loss.png                      — loss curve (required for judging)
...
def train_policy() -> None:
    ...
    Tuning knobs (env vars) so judges can see the loss curve finish on T4:
      NUM_EPOCHS         (default 1 for quick-mode, was 3)
      NUM_GENERATIONS    (default 4, was 8 — halves per-step reward evals)
      PER_DEVICE_BATCH   (default 2)
    """
    from train_grpo import train as grpo_train

    n_epochs = int(os.environ.get("NUM_EPOCHS", "1"))
    n_gens = int(os.environ.get("NUM_GENERATIONS", "4"))
    batch = int(os.environ.get("PER_DEVICE_BATCH", "2"))

    print(f"\n[train] GRPO on role={ROLE} model={MODEL_NAME} "
          f"epochs={n_epochs} group={n_gens} batch={batch}")
    grpo_train(
        role=ROLE,
...
def emit_loss_curve() -> None:
    """Extract loss history from TRL's trainer_state.json and emit training_loss.png.

    Required by hackathon automated-round validation. TRL writes
    trainer_state.json alongside the final checkpoint (or in the latest
    checkpoint-* subdir); log_history is a list of per-log-step dicts.
    """
    import matplotlib
    matplotlib.use("Agg")
    ...
    candidates += sorted(CKPT_DIR.glob("checkpoint-*/trainer_state.json"))
    state_path = next((p for p in candidates if p.exists()), None)
    if state_path is None:
        print("[loss-curve] no trainer_state.json found — skipping loss plot")
        return
    ...
    for row in history:
        ...
        if "reward" in row:
            rewards.append((s, float(row["reward"])))

    if not losses:
        print("[loss-curve] log_history had no 'loss' entries — skipping")
        return

    fig, ax1 = plt.subplots(figsize=(10, 5))
    ax1.plot(steps, losses, color="#C44E52", linewidth=2, label="train loss")
    ax1.set_xlabel("Training step")
    ax1.set_ylabel("Loss", color="#C44E52")
    ax1.tick_params(axis="y", labelcolor="#C44E52")
    ax1.grid(alpha=0.3)
    ax1.set_title(f"GRPO Training Loss — role={ROLE} model={MODEL_NAME}")

    if rewards:
        ax2 = ax1.twinx()
        rs, rv = zip(*rewards)
        ax2.plot(rs, rv, color="#4C72B0", linewidth=2,
                 marker="o", markersize=3, alpha=0.8, label="mean reward")
        ax2.set_ylabel("Mean reward", color="#4C72B0")
        ax2.tick_params(axis="y", labelcolor="#4C72B0")

    fig.tight_layout()
    out = ROOT / "training_loss.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    print(f"[loss-curve] wrote {out} ({len(losses)} loss points, "
          f"{len(rewards)} reward points)")
